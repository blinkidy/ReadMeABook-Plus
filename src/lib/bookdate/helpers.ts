...
export async function getUserLibraryBooks(
  ...
  try {
    ...
    // Build query filters. Full-library context includes BookOrbit ebooks;
    // rated scope stays on the audiobook library because BookOrbit rows do not
    // carry Plex user ratings.
    const whereClause: {
      plexLibraryId: string | { in: string[] };
      userRating?: { not: null };
    } = {
      plexLibraryId: backendMode === 'plex' && scope === 'rated' ? libraryId : { in: libraryIds },
    };
    let takeLimit = 40;
