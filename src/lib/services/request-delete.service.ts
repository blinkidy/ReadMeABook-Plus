...
export async function deleteRequest(
  ...
  try {
    ...
    try {
      ...
      const storedFormat = request.audiobook.fileFormat?.toLowerCase() || null;
      const ebookFormats = new Set(['epub', 'pdf', 'mobi', 'azw', 'azw3', 'fb2', 'cbz', 'cbr']);
      const storedPathMatchesRequest = isEbook
        ? storedFormat !== null && ebookFormats.has(storedFormat)
        : storedFormat === null || !ebookFormats.has(storedFormat);
      ...
      if (!titleFolderPath) {
      ...
      } else if (!storedPathMatchesRequest) {
        logger.warn(
          `Skipping media file deletion for request ${requestId}: stored ${storedFormat} path belongs to the other media type`
        );
      } else {
        ...
        try {
          ...
          if (isEbook) {
            ...
            const ebookExtensions = [...ebookFormats].map((format) => `.${format}`);
