...
describe('deleteRequest stored media path', () => {
  ...
  it('does not delete an ebook path when deleting the shared audiobook request', async () => {
    const ebookPath = path.join(tempRoot, 'BookOrbit', 'The Valley of Fear');
    const ebookFile = path.join(ebookPath, 'book.epub');

    await fs.mkdir(ebookPath, { recursive: true });
    await fs.writeFile(ebookFile, 'organized ebook');

    prismaMock.request.findFirst.mockResolvedValue({
      id: 'req-audiobook-shared-record',
      type: 'audiobook',
      audiobook: {
        id: 'ab-shared-record',
        title: 'The Valley of Fear',
    ...
    const { deleteRequest } = await import('@/lib/services/request-delete.service');
    const result = await deleteRequest('req-audiobook-shared-record', 'admin-1');

    expect(result.success).toBe(true);
    expect(result.filesDeleted).toBe(false);
    await expect(fs.readFile(ebookFile, 'utf8')).resolves.toBe('organized ebook');
  });
});
