...
3. **Delete Media Files**
   ...
   - Verifies the stored `fileFormat` matches the request type before deleting; shared audiobook/EPUB records otherwise skip cleanup
...
18. ✅ **Shared audiobook/EPUB record points at the other format** - Skips file deletion instead of removing the wrong library directory
...
- **Stored Path Only:** File cleanup targets a format-compatible path recorded by the organizer; no template-based fallback
